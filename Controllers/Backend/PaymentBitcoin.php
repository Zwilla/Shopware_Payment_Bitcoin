...
class Shopware_Controllers_Backend_PaymentBitcoin extends Shopware_Controllers_Backend_ExtJs
{
    /**
    ...
    public function getListAction()
    {
        ...
        $select = $this->get('db')
            ...
            ->from(array('o' => 's_order'), array(
                ...
                'total_paid_in_satoshi' => new Zend_Db_Expr('(' . $this->get('db')
                        ->select()
                        ->from(array('zwilla_free_bitcoin_transaction'), array('SUM(value_in_satoshi)'))
            ...
            ->joinLeft(
                array('btca' => 'zwilla_free_bitcoin_address'),
                'btca.address = o.transactionID',
                array(
                    'btcStatus' => 'btca.status', 'valueInBTC' => 'btca.value_in_BTC'
                )
            )
            ->joinLeft(
            ...
            ->where('p.name LIKE ?', 'zwillaweb_payment_bitcoin')
            ->where('o.status >= 0')
        ...
        foreach ($rows as &$row) {
            ...
            $row['total_paid_in_btc'] = $row['total_paid_in_satoshi'] / 100000000;
        }
    ...
    /**
     * Will register the correct shop for a given bitcoin_address.
     *
     * @param $bitcoin_address
     */
    public function registerShopBybitcoin_address($bitcoin_address)
    {
        ...
        ';
        $result = $this->get('db')->fetchOne($sql, array($bitcoin_address));
    ...
    public function getDetailsAction()
    {
        ...
        if (isset($filter[0]['property']) && $filter[0]['property'] == 'bitcoin_address') {
            $this->Request()->setParam('bitcoin_address', $filter[0]['value']);
        }
        $bitcoin_address = $this->Request()->getParam('bitcoin_address');

        // Load the correct shop in order to use the correct api credentials
        $this->registerShopBybitcoin_address($bitcoin_address);

        $select = $this->get('db')
            ->select()
            ->from(array('zwilla_free_bitcoin_transaction' => 'zwilla_free_bitcoin_transaction'), array(
        ...
        foreach ($rows as &$row) {
            $nr++;
            $row['number'] = $nr;
            $row['value_in_btc'] = $row['value_in_satoshi'] / 100000000;
        }
    ...
    {
        $this->View()->loadTemplate("backend/payment_bitcoin/app.js");
