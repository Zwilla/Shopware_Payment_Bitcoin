...
class Shopware_Controllers_Frontend_PaymentBitcoin extends Shopware_Controllers_Frontend_Payment implements \Shopware\Components\CSRFWhitelistAware
{
    /**
    ...
    /**
     * Whitelist notify- and webhook-action for bitcoin
     */
    ...
    {
        if ($this->getPaymentShortName() == 'zwillaweb_payment_bitcoin') {
        ...
        if ($user !== null && $amountdue > 0.15) {
            ...
            if (!in_array($currency_iso_code, $limited_currencies)) {
                ...
                if ($value_in_BTC > 0) {
                    ...
                    $zw_blockchain_api_key = Shopware()->Config()->getByNamespace('ZwWebPaymentBitcoin', 'zw_blockchain_api_key');
                    $zw_callback_secret = Shopware()->Config()->getByNamespace('ZwWebPaymentBitcoin', 'zw_callback_secret');

                    $config_secret = trim($zw_callback_secret);
                    $secret = strtoupper(md5($config_secret.'-'.$this->session->sUserId));

                    $url = Shopware()->Router()->assemble(array('action' => 'notify', 'controller' => 'PaymentBitcoin', 'module' => 'frontend', 'secret' => $secret, 'forceSecure' => true));
                    ...
                    if ($address) {
                        ...
                        Shopware()->Db()->exec("INSERT INTO `zwilla_free_bitcoin_address` 
                                (`id_order`,`value_in_BTC`,`address`,`status`,`crdate`) 
                            VALUES
                                ('".(int)$orderId."', '".(double)$value_in_BTC."', '".$address."', 'Pending', CURRENT_TIMESTAMP)");

                        $order->setComment('Please pay ' . $value_in_BTC . ' BTC to this address ' . $address . ' for order number ' . $orderNumber);

                        Shopware()->Models()->flush($order);
                        $this->View()->receivedAddress = 'YES';
                        $this->View()->bitcoinAddress = $address;
                        $this->View()->valueInBTC = $value_in_BTC;
        ...
        if ($result)
        {
            ...
            if ($secret_sent === $secret)
            {
                if ($confirmations <= 5)
                {
                    ...
                    Shopware()->Db()->exec("UPDATE `zwilla_free_bitcoin_address` SET `status` = 'AwaitingConfirmations' WHERE `address` = '".$address."'");
                    Shopware()->Db()->exec("UPDATE `zwilla_free_bitcoin_transaction` SET `confirmations` = '".(int)$confirmations."' WHERE `transaction_hash` = '".$transaction_hash."'");
